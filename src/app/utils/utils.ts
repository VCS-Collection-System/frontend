...
export function covidVaxOneDateRules(date, earliestShotDate) {
  const covidVaxOneDate = new Date(date.replace(/-/g,'/'));
  return covidVaxOneDate <= new Date() && covidVaxOneDate >= new Date(earliestShotDate);
}

export function covidVaxDateValidator(date, earliestShotDate, shotOneDate) {
  const today = new Date();
  const minDate = new Date(earliestShotDate);
...
export function covidVaxDateRules(date, earliestShotDate, prevShotDate) {
  if(!date && prevShotDate) {
    return true; 
  }
  else {
    const earliestVaxDate = new Date(earliestShotDate);
    const covidVaxDate = new Date(date.replace(/-/g,'/'));

    if(prevShotDate) {
      const covidVaxPrevDate = new Date(prevShotDate.replace(/-/g,'/'));
      return covidVaxDate <= new Date() && covidVaxDate >= earliestVaxDate && covidVaxDate > covidVaxPrevDate;
    }

    return covidVaxDate <= new Date() && covidVaxDate >= earliestVaxDate;
  }
}